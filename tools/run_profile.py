...
if __name__ == "__main__":
    ...
    parser.add_argument('-k', '--checkpoint', type=str, default=None, help='optional checkpoint path')
    parser.add_argument('--shape', type=int, nargs='+', default=[1, 3, 640, 640],
                        help='input shape (default: 1 3 640 640). If two values given, treated as H W with batch=1, channels=3')
    args = parser.parse_args()

    if len(args.shape) == 2:
        shape = [1, 3, args.shape[0], args.shape[1]]
    else:
        shape = args.shape
    ...
    stats = profile_stats(model, shape=shape, verbose=True)
    print(f"\nSummary: Params={stats['n_parameters']:,} | FLOPs={stats['n_flops']:.2f} with shape {shape}")
