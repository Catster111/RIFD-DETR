...
from .rtdetr_keypoint_head import RTDETRKeypointHead
...
class RTDETRTransformerv2(nn.Module):
    ...
    def __init__(self,
        ...
        # keypoint head (only if enabled)
        if self.use_keypoints:
            self.keypoint_head = RTDETRKeypointHead(
    ...
    def forward(self, feats, targets=None):
        ...
        # decoder
        out_bboxes, out_logits, decoder_features = self.decoder(
            init_ref_contents,
        ...
        if self.training and dn_meta is not None:
            ...
            # Also split decoder features if using keypoints
            if self.use_keypoints:
                dn_decoder_features, decoder_features = torch.split(decoder_features, dn_meta['dn_num_split'], dim=1)

        out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1]}
        
        # Add keypoint predictions if enabled
        if self.use_keypoints:
            pred_heatmaps, pred_offsets = self.keypoint_head(decoder_features)
            out['pred_keypoint_heatmaps'] = pred_heatmaps
            out['pred_keypoint_offsets'] = pred_offsets
