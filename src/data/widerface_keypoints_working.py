...
@register()
class WiderFaceKeypointDatasetWorking(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Some WiderFace JSON entries do not include width/height; fall back to actual image size
        ...
        target = {
            ...
            'size': torch.tensor([img_w, img_h])  # W, H
        }
        ...
        if self._transforms is not None:
            ...
            if isinstance(target.get('keypoints'), tv_tensors.Keypoints):
                coords = target['keypoints'].as_subclass(torch.Tensor)
                visibility = target.pop('keypoints_visibility', None)
                if visibility is None:
                    visibility = torch.ones(coords.shape[:-1])
                if visibility.ndim == 2:
                    visibility = visibility.unsqueeze(-1)
                target['keypoints'] = torch.cat([coords, visibility], dim=-1)
    ...
    def load_item(self, idx):
        ...
        img_path = os.path.join(self.img_prefix, file_name)
        image = Image.open(img_path).convert('RGB')
        # Safe width/height for evaluator paths as well
        img_w = img_meta.get('width')
        img_h = img_meta.get('height')
        if img_w is None or img_h is None:
            img_w, img_h = image.size
